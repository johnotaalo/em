		...
		<div class="col-12 col-lg-10 col-xl-8">
			<div class="header mt-md-5">
				...
				<b-form class="mb-4 mt-5" @submit.stop.prevent="onSubmit">
					...
					<div class="row mt-5">
						<div class="col-12 col-md-6">
							<button type="submit" class="btn btn-primary lift" :disabled="$v.form.$invalid">Edit User</button>
						</div>
					</div>
				</b-form>
			</div>
	...
	export default {
				...
				email: { required, email,
					isUnique(value){
						...
						return new Promise((resolve, reject) => {
							axios.post('/api/users/validate/email', { email: value, id: this.id })
							.then(res => {
								if (res.data.exists)
		...
		methods: {
			onSubmit: function(){
				...
				this.form.put(`/api/users/edit/${this.id}`)
				.then((response)=>{
					this.$swal({
						title: "Success!", 
				...
				.catch((error) => {
					this.$swal("Oops Something went wrong!", "There was an error while editing user. Counter check your information or contact administrator", "error")
				});
			},
			getData(id){
				...
				.then((res) => {
					...
					this.form.status = (user.status == 1) ? true : false
				})
			}
		}
	}
